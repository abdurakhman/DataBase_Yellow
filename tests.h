...
int DoRemove(Database& db, const string& s) {
    istringstream is(s);
...
string DoPrint(const Database& db) {
    ostringstream os;
    db.Print(os);
    return os.str();
}
...
void TestDatabaseLast() {
  {
    ...
    db.Last({ 2017, 11, 21 });
    AssertEqual(db.Last({ 2017, 11, 20 }), make_pair(Date(2017, 11, 20), "Monday"), "last case 1");
    AssertEqual(db.Last({ 2017, 11, 21 }), make_pair(Date(2017, 11, 21), "Weekly meeting"), "last case 2");
    try {
        db.Last({ 2000, 1, 1 });
    ...
    AssertEqual(db.Last({ 2017, 11, 21 }), make_pair(Date(2017, 11, 21), "Tuesday"), "last case 4");
    AssertEqual(db.Last({ 9999, 12, 31 }), make_pair(Date(2017, 11, 21), "Tuesday"), "last case 5");

    db.Add({ 2017, 11, 21 }, "Weekly meeting");

    AssertEqual(db.Last({ 2017, 11, 21 }), make_pair(Date(2017, 11, 21), "Weekly meeting"), "last case 6");
    AssertEqual(db.Last({ 9999, 12, 31 }), make_pair(Date(2017, 11, 21), "Weekly meeting"), "last case 7");
...
int DoRemove2 (Database& db, const string& str) {
...
void TestDbRemoveIf () {
    {
        ...
        db.Add({2017, 1, 1}, "new year");
        db.Add({2017, 1, 7}, "xmas");
        AssertEqual(0, DoRemove2(db, R"(event == "something")"), "Remove nothing");
        AssertEqual(1, DoRemove2(db, R"(date == "2017-01-01")"), "Remove by date");
        ostringstream out;
        db.Print(out);
    ...
    }
    {
        ...
        db.Add({2017, 1, 1}, "new year");
        db.Add({2017, 1, 7}, "xmas");
        AssertEqual(1, DoRemove2(db, R"(event == "xmas")"), "Remove by event");
        ostringstream out;
        db.Print(out);
    ...
    }
    {
        ...
        AssertEqual(2, DoRemove2(db, R"(event == "new year")"), "Multiple remove by event");
        ostringstream out;
        db.Print(out);
